...
func fill_array(in io.ByteReader, bs []byte) (err error) {
	for i := range bs {
...
func read_int16(in io.ByteReader) (out int16, err error) {
	var bs [2]byte
	bslice := bs[:]
...
func read_uint16(in io.ByteReader) (out uint16, err error) {
	var bs [2]byte
	bslice := bs[:]
...
func read_f32(in io.ByteReader) (out float64, err error) {
	var bs [4]byte
	bslice := bs[:]
...
func read_f64(in io.ByteReader) (out float64, err error) {
	var bs [8]byte
	bslice := bs[:]
