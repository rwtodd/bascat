...
static void decrypt_buffer (uint8_t *buf, size_t len)
{
	...
	*buf++ = 0xff;  /* don't need to decrypt again */
	...
	while (buf != end)
	{
		*buf = ((*buf - (11 - idx11)) ^ (KEY11[idx11]) ^ (KEY13[idx13])) +
			(13 - idx13);
		++buf;
		if (++idx11 == 11) idx11 = 0;
		if (++idx13 == 13) idx13 = 0;
...
// allocate memory in gwbas_data and fill it with the contents of the storage file
void fill_buffer (Windows::Storage::StorageFile &sf, gwbas_data *const b)
{
	auto buff = Windows::Storage::FileIO::ReadBufferAsync (sf).get ();
	auto dr = Windows::Storage::Streams::DataReader::FromBuffer (buff);
	b->index = 1;
	b->len = dr.UnconsumedBufferLength ();
	b->buffer = new uint8_t[b->len];
	dr.ReadBytes (array_view<uint8_t>{ b->buffer, b->buffer + b->len });
	dr.Close ();
}
...
{
	std::wostringstream fname_builder{};
	bool relative = ((strlen (fname) > 2) && (fname[0] == '.') && (fname[1] == '\\'));
	fname_builder << (fname + (relative ? 2 : 0));

	try
	{
		wchar_t dir[255];
		GetCurrentDirectory (255, dir);
		auto sfolder = Windows::Storage::StorageFolder::GetFolderFromPathAsync (dir).get ();
		auto sf = sfolder.GetFileAsync (fname_builder.str ()).get ();
		fill_buffer (sf, b);
	}
	catch (...)
	{
		...
		try
		{
			auto sf = Windows::Storage::StorageFile::GetFileFromPathAsync (fname_builder.str ()).get ();
			fill_buffer (sf, b);
		}
		catch (...)
		{
